    ...
    printWindow.document.write(`
      <html>
        ...
        <body class="bg-gray-100 flex flex-col items-center p-6">
          <h1 class="text-2xl font-semibold mb-6">Lista de movimientos</h1>
          <table class="w-4/5 max-w-4xl border-collapse shadow-lg bg-white">
            <thead class="bg-green-500 text-white">
              <tr>
                <th class="px-4 py-2 text-sm font-medium border">ID</th>
                <th class="px-4 py-2 text-sm font-medium border">Tipo</th>
                <th class="px-4 py-2 text-sm font-medium border">Categoría</th>
                <th class="px-4 py-2 text-sm font-medium border">Monto</th>
                <th class="px-4 py-2 text-sm font-medium border max-w-md truncate">Descripción</th>
                <th class="px-4 py-2 text-sm font-medium border">Fecha</th>
              </tr>
            </thead>
            <tbody class="text-sm">
              ${transactions
                .map((trx) => {
                  ...
                  return `
                    <tr class="odd:bg-gray-100 even:bg-white hover:bg-gray-200">
                      ...
                      <td class="border px-4 py-2">${trx.type === "income" ? "Ingreso" : "Gasto"}</td>
...
export const exportTransactions = async (
  ...
  const csvRows = transactions.map((trx): string => {
    const row = [
      trx.id ?? "",
      trx.date ?? "",
      (trx.type === "income" ? "ingreso" : trx.type === "expense" ? "gasto" : ""),
      trx.amount ?? "",
      trx.original_amount ?? "",
      trx.original_currency ?? "",
  ...
  const filename = `movimientos_${timestamp}.csv`;
...
export const downloadJSON = async (
  ...
  const timestamp = new Date().toISOString().replace(/[:.-]/g, "_");
  const filename = `movimientos_${timestamp}.json`;
     ...
     <html lang="es">
      ...
      <body>
        <div class="max-w-xs mx-auto p-4 bg-white border border-gray-300 shadow-md">
          <h2 class="text-lg font-bold text-center mb-2">Comprobante de movimiento</h2>
          <div class="border-b border-dashed border-gray-400 my-3"></div>

          <div class="text-sm space-y-1">
            <div class="flex justify-between">
              <strong>N.° de movimiento:</strong> <span>#${transaction.id}</span>
            </div>
            <div class="flex justify-between">
            ...
            <div class="flex justify-between">
              <strong>Fecha:</strong> <span>${new Date(transaction.date).toLocaleString("es-PE")}</span>
            </div>
            <div class="flex justify-between">
                <strong>Monto:</strong> <span class="uppercase">${originalInfo}${formattedAmount}</span>
            </div>
            <div class="flex justify-between">
              <strong>Categoría:</strong> <span>${transaction.category}</span>
            </div>
          ...
          <div class="text-xs text-gray-600 text-center mt-2">
            Generado desde tu administrador de gastos.
          </div>
        </div>
      </body>
    </html>
